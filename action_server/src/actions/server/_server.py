...
def _reload_action_generation(action_routes, actions_process_pool, actions, packages):
    ...
    The process pool is prepared first, while the old routes remain exposed.
    Route state is restored if registration fails, and the pool is rolled back
    to the old action generation before the failure is returned to the watcher.
    The lock makes the state transition one critical section for reload callers.
    """
