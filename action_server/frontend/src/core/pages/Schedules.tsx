...
import { getStaggerDelay } from '@/shared/utils/animations';
import { copyToClipboard, downloadAsJson } from '@/shared/utils/helpers';
...
export function SchedulesPage() {
  ...
  // Track if component has mounted to prevent re-animation on filter/selection changes
  ...
  const handleCopySchedule = async (schedule: Schedule) => {
    const config = {
      ...
      weekday_config: schedule.weekday_config,
  ...
  const handleExportSchedule = (schedule: Schedule) => {
    downloadAsJson(schedule, `schedule-${schedule.name.toLowerCase().replace(/\s+/g, '-')}`);
  };

  const handleExportAllSchedules = () => {
    if (filteredSchedules.length > 0) {
      downloadAsJson(filteredSchedules, 'schedules-export');
    }
  };
      ...
      <div className="rounded-lg border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-4 border-b border-border p-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex w-full gap-2 sm:w-auto">
            ...
            <Button variant="outline" onClick={handleExportAllSchedules} disabled={filteredSchedules.length === 0}>
              Export All
            </Button>
              ...
              <TableBody>
                {filteredSchedules.map((schedule, index) => (
                  <TableRow
                    ...
                    style={!hasMounted.current ? getStaggerDelay(index, 30) : undefined}
                  >
                    <TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleCopySchedule(schedule)}>
                              {copiedId === schedule.id ? 'Copied!' : 'Copy Config'}
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleExportSchedule(schedule)}>
                              Export JSON
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
