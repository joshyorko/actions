/* eslint-disable react/prop-types -- props are typed by React's TypeScript attributes. */
...
>(({ className, ...props }, ref) => (
  <div
    className="w-full max-w-full overflow-auto overflow-x-auto"
    tabIndex={0}
    aria-label="Scrollable table"
  >
    <table
      ref={ref}
      className={cn("w-full min-w-[32rem] caption-bottom text-sm", className)}
      {...props}
    />
  </div>
));
...
>(({ className, ...props }, ref) => (
  ...
  <thead
    ref={ref}
    className={cn(
      "[&_tr]:border-b [&_tr]:border-border [&_tr]:bg-muted/50",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn("bg-muted font-medium text-card-foreground", className)}
    {...props}
  />
));
...
const TableEmptyState = ({
  ...
  <div
    className={cn(
      "flex items-center justify-center p-12 text-center text-sm text-muted-foreground",
      className,
    )}
  >
    {children}
