...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Close
    ref={ref}
    className={cn(
      "absolute top-4 right-4 rounded-md bg-transparent p-1 text-muted-foreground transition-colors motion-reduce:transition-none",
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-lg max-h-[calc(100vh-2rem)] overflow-auto -translate-x-1/2 -translate-y-1/2",
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "border-t border-border pt-4 mt-4 flex justify-end gap-2",
      className,
    )}
    {...props}
  />
