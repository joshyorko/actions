        ...
        <div className="space-y-6">
          ...
          {scheduleType === 'once' && (
            <div className="space-y-4">
              <div>
                <label htmlFor="once-date" className="block text-sm font-medium text-card-foreground mb-2">
                  Date
                </label>
                <Input
                  id="once-date"
                  type="date"
                  value={onceAt ? onceAt.split('T')[0] : ''}
                  onChange={(e) => {
                    const date = e.target.value;
                    const time = onceAt?.split('T')[1] || '12:00';
                    onOnceAtChange(date ? `${date}T${time}` : undefined);
                  }}
                />
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    value={onceAt ? parseInt(onceAt.split('T')[1]?.split(':')[0] || '12', 10) : 12}
                    onChange={(e) => {
                      const hour = e.target.value.padStart(2, '0');
                      const date = onceAt?.split('T')[0] || new Date().toISOString().split('T')[0];
                      const minute = onceAt?.split('T')[1]?.split(':')[1] || '00';
                      onOnceAtChange(`${date}T${hour}:${minute}`);
                    }}
                  >
                  ...
                  <select
                    className="flex h-10 w-24 rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                    value={onceAt ? parseInt(onceAt.split('T')[1]?.split(':')[1] || '0', 10) : 0}
                    onChange={(e) => {
                      const minute = e.target.value.padStart(2, '0');
                      const date = onceAt?.split('T')[0] || new Date().toISOString().split('T')[0];
                      const hour = onceAt?.split('T')[1]?.split(':')[0] || '12';
                      onOnceAtChange(`${date}T${hour}:${minute}`);
                    }}
                  >
