...
const canvasHtml = readFileSync(
  resolve(frontendRoot, "apps/canvas-view/index.html"),
  "utf8",
);
const canvasMain = readFileSync(
  resolve(frontendRoot, "apps/canvas-view/src/main.tsx"),
  "utf8",
);
...
describe("owned UI system contract", () => {
  ...
  it("defines offline type roles and semantic motion contracts", () => {
    expect(css).toMatch(/--font-sans:/);
    expect(css).toMatch(/--font-mono:/);
    expect(css).toMatch(/--motion-interaction:/);
    expect(css).toMatch(/--motion-disclosure:/);
    expect(css).toMatch(/--motion-pane:/);
    expect(css).toMatch(/--motion-trace:/);
    expect(css).not.toMatch(
      /fonts\.googleapis\.com|linear-gradient|backdrop-blur/,
    );
  });
  ...
  it("keeps muted text above WCAG AA on light and dark semantic surfaces", () => {
    for (const tokenReader of [lightToken, darkToken]) {
      ...
      for (const surface of ["background", "card", "muted"]) {
        expect(
          contrastRatio(mutedForeground, parseHsl(tokenReader(surface))),
