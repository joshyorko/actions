...
from actions import _constants
from actions._constants import SUPPORTED_TYPES_IN_SCHEMA
...
if typing.TYPE_CHECKING:
    from actions._action_context import RequestContexts
...
def metadata(
    *,
    ...
    from actions._action import Context
...
def _os_exit(retcode: int):
    ...
    try:
        ...
        try:
            ...
            log.info(f"actions-core killing processes after run: {names}")
...
def run(
    *,
    ...
    """
    from actions._action import (
        set_current_action_context,
        set_current_requests_contexts,
    )
    from actions._collect_actions import update_pythonpath
    from actions._response import ActionError, Response
    ...
    from actions._action import Context, set_current_action
    from actions._collect_actions import collect_actions
    from actions._config import RunConfig, set_config
    from actions._exceptions import ActionsCollectError
    ...
    try:
        ...
        if os_exit_enum != _OsExit.NO:
            ...
            log.info(f"actions: os._exit option: {os_exit}")
            _os_exit(retcode)
        ...
        if os.environ.get(var_name_dump_threads, "1").lower() not in (
            ...
            from actions._interrupts import dump_threads
